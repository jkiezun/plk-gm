...
class Player(models.Model):
    ...
    fantasy_club = models.ManyToManyField(
        FantasyClub, through="ClubMember", blank=True, related_name="players"
    )
    ...
    position = models.CharField(max_length=2, choices=POSITIONS, default="PG")
...
class ClubMember(models.Model):
    ...
    fantasy_club = models.ForeignKey(
        FantasyClub, on_delete=models.CASCADE, related_name="club_members"
    )
...
class PlayerStat(models.Model):
    ...
    def get_fantasy_points(self):
        score = (
            float(self.points)
            + float(self.defreb)
            + float(self.offreb * 1.25)
            + float(self.assists * 1.5)
            + float(self.steals * 1.5)
            - float(self.turnovers * 1.5)
            + float(self.blocks * 1.5)
            - float((self.fga - self.fgm))
            - float(self.fouls)
        )
        return score
